	...
	--------------------
		...
		5.2		---	CHANGES
					--	In detectCone1, before finding 10 largest contours in terms of area,
						...
						can't be cones)
					--	detectCone1 should return a vector of rectangles instead of an image.
...
//	FUNCTIONS
std::pair<vector<Rect>, Mat> detectCone1(Mat img, int col1, int col2, Mat temp);
Rect detectCone2(Mat img, int col1, int col2);
Color loadColorParameters(int col);
Mat drawCones(vector<Rect> coneVect, Mat img);
...
// GLOBAL VAR Publisher
image_transport::Publisher pub;

void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
	////Test code to show image only
  // try
  // {
  ...
  // }
	...
	//Vector of Rectangles/ Bounding Boxes
	std::pair<vector<Rect>, Mat> yellowCones, orangeCones, blueCones;

#if(STREAMSOURCE == 0)
	//If you just want to work with one image only, load image here
	cameraFeed = imread("Cone7.jpg", CV_LOAD_IMAGE_COLOR);
#elif(STREAMSOURCE == 1)
	...
	*/
	...
	FRAME_HEIGHT = cameraFeed.rows;
		...
		if (!cameraFeed.empty())
		{
			...
			// pub.publish(msg);
			sensor_msgs::ImagePtr msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", temp_clone).toImageMsg();
			pub.publish(msg);
			ROS_INFO("Publisher !");
		}
#endif
...
// //Mat cameraFeed;
// namespace enc = sensor_msgs::image_encodings;
// void zed_callback(const sensor_msgs::ImageConstPtr& msg)
// {
// 	ROS_INFO("Subscriber IN !");
//   cv_bridge::CvImageConstPtr cv_ptr;
//   try
...
int main(int argc, char* argv[])
{
	ros::init(argc, argv, "image_publisher");
  	ros::NodeHandle nh;
  	image_transport::ImageTransport it_p(nh);
		image_transport::ImageTransport it_s(nh);
  	pub = it_p.advertise("ajinkya/image", 1);
		sensor_msgs::ImagePtr msg;
	ros::Rate loop_rate(30);
  // image_transport::Subscriber sub = it_s.subscribe("/zed/rgb/image_rect_color", 1, zed_callback);
	ROS_INFO("Init !");
	image_transport::ImageTransport it_test(nh);
  image_transport::Subscriber sub2 = it_test.subscribe("/zed/rgb/image_rect_color", 1, imageCallback);
	ros::spin();
...
std::pair<vector<Rect>, Mat> detectCone1(Mat temp, int col1, int col2, Mat temp_clone)
{
	//clone original image
	...
	// Vector to store resulting rectangles
	vector<Rect> resultingRect;
	...
	if (contours.size() != 0)
	{
		...
		std::vector<std::vector<int>> mergeIndex(contourSize, std::vector<int>(contourSize, 0));
		...
		{
			Rect box = boundingRect(Mat(contours[indices[ind1]]));
...
Mat drawCones(vector<Rect> coneVec, Mat img)
{
